/*
INIT FAITE DANS init_load_recherche.js
window.onload = function(){initAutoComplete(document.getElementById('form_recherche'),
document.getElementById('vauteur'),document.getElementById('bouton-submit'))};
*/
window.onload = function(){

	initAutoCompleteTaxonomy(document.getElementById('form_saisie_noms'),document.getElementById('nom_scientifique'));

}

function focus_taxonomy (etat) {
	//if (_completeDiv.style.visibility=="hidden") {  _inputField.value = ""; }
}
...
var _hiddenFieldSql=null; // le champs cache avec la requete sql en cours

function initAutoCompleteTaxonomy(form,field){
  _minimum_caractere = 3;
  _documentForm=form;
  _inputField=field;
  _submitButton=null;
...
var _adresseRecherche = "/wp-content/plugins/iherbarium/completion/completion_taxonomy.php" //l'adresse à interroger pour trouver les suggestions
...
function callSuggestions(valeur){
  ...
  _xmlHttp=getXMLHTTP();
  if(_xmlHttp){
	 if (valeur.length < _minimum_caractere) { 
	   _completeDiv.style.visibility="hidden";
	 } else {
		genre_obs = document.getElementById('genre_obs').value;
		limite_france = document.getElementById('limite_france').checked;
		if (limite_france==true){
			limite_france=1;
		}else{
			limite_france=0;
		}
       //appel à l'url distante
	   if(_hiddenFieldSql!=null) { _xmlHttp.open("GET",_adresseRecherche+"?debut="+valeur+"&genre_obs="+genre_obs+"&limite_france="+limite_france+"&req="+_hiddenFieldSql,true); }
	   else { _xmlHttp.open("GET",_adresseRecherche+"?debut="+valeur+"&genre_obs="+genre_obs+"&limite_france="+limite_france,true); }
...
function calculateWidth(){
  return _inputField.offsetWidth-2*1 + 100
}
...
//	 _completeDiv.style.width=calculateWidth()+"auto";
  }
}
...
function creeAutocompletionDiv() {
  ...
  _completeDiv.style.borderRight="#B4B0AF "+borderLeftRight+"px solid";
  _completeDiv.style.borderLeft="#B4B0AF "+borderLeftRight+"px solid";
  _completeDiv.style.borderTop="#B4B0AF "+borderTopBottom+"px solid";
  _completeDiv.style.borderBottom="#B4B0AF "+borderTopBottom+"px solid";
  _completeDiv.style.zIndex="1";
  _completeDiv.style.paddingRight="0";
  _completeDiv.style.paddingLeft="0";
  _completeDiv.style.paddingTop="0";
  _completeDiv.style.paddingBottom="0";
  _completeDiv.style.textAlign="left";
  setCompleteDivSize();
  _completeDiv.style.visibility="hidden";
...
function metsEnPlace(valeur, liste){
  ...
  if(_completeDivRows>0) {
    _completeDiv.height=14*_completeDivRows+4;
  } else {
    hideCompleteDiv();
  }
